...
//
//        time = time/number;
//    String cmd = "cd "+ query+".au; cat " +s.toString()+ "| ffmpeg -framerate "+time+" -f image2pipe -i - -vf \"scale=iw*min(1920/iw\\,1080/ih):ih*min(1920/iw\\,1080/ih), pad=1920:1080:(1920-iw*min(1920/iw\\,1080/ih))/2:(1080-ih*min(1920/iw\\,1080/ih))/2,format=yuv420p,drawtext=fontfile=myfont.ttf:fontsize=100: fontcolor=white:shadowx=2:x=(w-text_w)/2:y=(h-text_h)/2:text='"+query+"'\" -r 25 "+query+".mkv";
//
//        pbuilder.getInstance().probuild(cmd);
//
//
//        String cmd7 = "cd "+ query+".au; ffmpeg -i "+query+".mkv -i " + _list.getSelectionModel().getSelectedItem().toString() + ".wav -c copy " + _creationName.getText()+".mkv";
//
...
//                double durationInSeconds = (frames + 0.0) / format.getFrameRate();
    ...
    public void CombineVideo(double time) throws IOException {
        ...
        String cmd7 = "cd "+ query+".au; ffmpeg -i "+query+".mp4 -i " + _list.getSelectionModel().getSelectedItem().toString() + ".wav -vcodec copy -strict -2 " + _creationName.getText()+".mp4";

        pbuilder.getInstance().probuild(cmd7);

        String cmd8 = "cd "+ query+".au; "+"mv "+_creationName.getText()+".mp4 /afs/ec.auckland.ac.nz/users/m/s/msid633/unixhome/IdeaProjects/se206A3/Creations";
...
//finish
    ...
    public void switchBack() {
        ...
        try {
            sw.switchScenes("Directory.fxml");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
